...
struct AVXDiv : ::testing::TestWithParam<std::tuple<
    double, // min value
    double,  // max value
    test::vector_t
    >>
...
void DoTest(const double min_val, const double max_val)
{
    ...
    // do scalar operation
    auto scal_Div= [](d_t in1, d_t in2) { return in1 / in2; };
    scal1.Operation(scal2, scal_dst, scal_Div);
